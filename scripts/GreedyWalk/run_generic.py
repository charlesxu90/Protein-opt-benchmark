#!/usr/bin/env python
"""
run_generic.py - Greedy hill-climbing baseline for protein optimization benchmark

Performs greedy hill climbing with random restarts on the fitness landscape.
Each round: pick the best sequence so far, mutate it at each position, evaluate
all single-mutant neighbors, and select the top batch_size. Serves as an upper
baseline that exploits the landscape without a learned model.
...
def greedy_walk(
    ...
    Rounds 1+: for the best sequence found so far, enumerate single-mutant
    neighbors that exist in the landscape. Pick the top batch_size by fitness.
    If fewer neighbors are available, fill with random unexplored sequences.
    """
    n_total = len(all_sequences)
    queried = set()
    all_indices = []

    # Round 0: random initialization
    init_indices = rng.choice(n_total, size=min(batch_size, n_total), replace=False)
    for idx in init_indices:
        queried.add(idx)
    all_indices.extend(init_indices.tolist())

    for rnd in range(1, n_rounds):
        # Find best sequence so far
        queried_fitness = all_fitness[all_indices]
        best_idx = all_indices[np.argmax(queried_fitness)]
        best_seq = all_sequences[best_idx]

        # Enumerate single-mutant neighbors in the landscape
        neighbor_indices = []
        for pos in range(len(best_seq)):
            for aa in AMINO_ACIDS:
                if aa == best_seq[pos]:
                    continue
                mutant = best_seq[:pos] + aa + best_seq[pos + 1:]
                if mutant in seq_to_idx:
                    idx = seq_to_idx[mutant]
                    if idx not in queried:
                        neighbor_indices.append(idx)

        # Sort neighbors by fitness (greedy)
        if neighbor_indices:
            neighbor_indices = np.array(neighbor_indices)
            neighbor_fitness = all_fitness[neighbor_indices]
            sorted_order = np.argsort(-neighbor_fitness)
            selected = neighbor_indices[sorted_order[:batch_size]]
        else:
            selected = np.array([], dtype=int)

        # Fill remaining slots with random unexplored sequences
        n_remaining = batch_size - len(selected)
        if n_remaining > 0:
            unexplored = np.array([i for i in range(n_total) if i not in queried])
            if len(unexplored) > 0:
                fill = rng.choice(unexplored, size=min(n_remaining, len(unexplored)), replace=False)
                selected = np.concatenate([selected, fill]) if len(selected) > 0 else fill

        for idx in selected:
            queried.add(int(idx))
        all_indices.extend(selected.astype(int).tolist())

        print(f"  Round {rnd}: {len(neighbor_indices) if neighbor_indices is not None else 0} neighbors found, "
              f"best fitness so far: {all_fitness[all_indices].max():.4f}")
...
def run_single_experiment(
    ...
    if output_path is None:
        output_path = os.path.join(BENCHMARK_ROOT, "GreedyWalk", "results", f"{dataset}_GreedyWalk")
    ...
    # convention used by AiCE/ALDE/FLEXS/EvoPlay/alphavariant. The greedy
    # algorithm's relative rankings are unaffected by this rescaling.
    _global_max_raw = float(np.max(all_fitness_raw))
    all_fitness = all_fitness_raw / _global_max_raw
    seq_to_idx = {seq: i for i, seq in enumerate(all_sequences)}

    print(f"\n{'='*60}")
    print(f"Greedy Walk Baseline on {dataset}")
    print(f"  Seed: {seed}, Batch: {batch_size}, Rounds: {n_rounds}")
    print(f"  Landscape size: {len(all_sequences)}, Seq length: {len(all_sequences[0])}")
    print(f"{'='*60}")

    start_time = datetime.now()
    sampled_indices = greedy_walk(
        all_sequences, all_fitness, seq_to_idx, batch_size, n_rounds, rng)
    ...
    # Save indices
    subdir = os.path.join(output_path, dataset, "greedy", "")
    os.makedirs(subdir, exist_ok=True)
    indices_path = os.path.join(subdir, f"GreedyWalk_seed{seed}_indices.pt")
...
def main():
    ...
    if args.output_path is None:
        args.output_path = os.path.join(BENCHMARK_ROOT, "GreedyWalk", "results", f"{args.dataset}_GreedyWalk")
    ...
    print(f"GreedyWalk baseline on {args.dataset} with {len(seeds)} seed(s): {seeds}")
