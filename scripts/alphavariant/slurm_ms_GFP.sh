...
#SBATCH --mem=64G
#SBATCH --time=05:00:00                     # 5 packed GFP seeds (237aa, ~110 min/seed) on one A100
#SBATCH --output=results_oracle/_logs/slurm/av_gfp_%A_%a.out
# ---- EDIT for your cluster ----
###SBATCH --partition=batch
###SBATCH --account=YOUR_ACCOUNT
###SBATCH --constraint=a100
#
# AlphaVariant (Plan C) on ms_GFP, multi-site ORACLE benchmark, packed for 80GB A100.
...
#
# PACKING: one AlphaVariant seed uses only ~2GB / ~20% of an A100-80G (sampling-latency
...
#   oracles/ms_GFP/oracle.pt
#   alphavariant/priors/ms_GFP/prior_model.{pt,json}
#   data/ms_GFP/{data.csv,wt.fasta,mutcompute.csv}
#   conda env `alphavariant-env` (torch+transformers+sklearn+loguru+lightning+popgen)
#
# Submit:  sbatch scripts/alphavariant/slurm_ms_GFP.sh
...
DATASET="ms_GFP"
SEEDS_PER_TASK="${SEEDS_PER_TASK:-5}"       # concurrent seeds per GPU (raise on 80G if throughput allows)
N_ROUNDS="${N_ROUNDS:-5}"
