#!/usr/bin/env python
"""
run_generic.py - Execute delta-Conservative Search optimization on any dataset

Generic version of the dataset-specific runners. Accepts --dataset as a required
CLI argument and auto-detects sequence length and wildtype from the data.
...
Based on delta-Conservative Search method adapted for arbitrary protein variant landscapes.
...
Usage:
    ...
    python run_generic.py --dataset AAV_hard --seed 42

    # Multiple runs with different seeds
    python run_generic.py --dataset GFP_med --seeds 42 123 456 789 1000

    # Use seeds from file
    python run_generic.py --dataset GB1 --seed_file ../rand_seeds.txt --num_seeds 50

    # Custom output path
    python run_generic.py --dataset AAV_med --output_path results/my_experiment/
"""
...
parser = argparse.ArgumentParser(
    description="Run delta-Conservative Search optimization on any dataset",
...
parser.add_argument("--output_path", default=None, type=str,
                    help="Output directory (default: results/<dataset>_delta_cs/)")
parser.add_argument("--save_path", default=None,
                    help="Save path for logger (default: <output_path>/<dataset>_delta_cs.pkl.gz)")
parser.add_argument("--tb_log_dir", default=None,
                    help="Tensorboard log dir (default: <output_path>)")
parser.add_argument("--name", default=None,
                    help="Experiment name (default: <dataset>_delta_cs)")
...
parser.add_argument("--task", default=None, type=str, help="Task name (default: derived from dataset)")
...
parser.add_argument("--max_len", default=None, type=int,
                    help="Sequence length (auto-detected from data if not specified)")
parser.add_argument("--gen_max_len", default=None, type=int,
                    help="Generator max length (defaults to max_len)")
...
# ============================================================================
# Generic Oracle and Dataset
# ============================================================================

class GenericOracle:
    """Oracle wrapper for any fitness landscape dataset."""

    def __init__(self, dataset_name: str, data_dir: str = '/home/xux/Desktop/AlphaVariant/Benchmark/data'):
    ...
    def _load_landscape(self):
        ...
        data_path = os.path.join(self.data_dir, self.dataset_name, 'data.csv')
        ...
        if seq_col is None:
            raise ValueError(
                f"Could not find sequence column in {data_path}. "
        ...
        print(f"Loaded {self.dataset_name} landscape: {len(self.sequences)} sequences")
        print(f"  Fitness range: [{self.min_fitness:.4f}, {self.max_fitness:.4f}]")
        print(f"  Sequence length: {self.seq_length}")
        print(f"  Wildtype (best fitness): {self.wildtype[:50]}{'...' if len(self.wildtype) > 50 else ''}")
...
class GenericDataset:
    """Dataset class for generic protein optimization."""

    def __init__(self, args, oracle: GenericOracle, init_size: int = 96):
        self.args = args
        self.oracle = oracle
...
def get_tokenizer(max_len):
    """Create tokenizer for the given sequence length."""
    return GenericTokenizer(max_len=max_len)
...
def compute_metrics(dataset: GenericDataset, oracle: GenericOracle, new_batch, round: int,
...
def train(args, oracle, dataset):
    """Main training loop for delta-CS on a generic dataset."""
    tokenizer = get_tokenizer(args.max_len)
...
def run_single_experiment(seed: int, args, output_path: str) -> Dict[str, Any]:
    ...
    print(f"\n{'='*60}")
    print(f"Starting delta-CS optimization on {dataset_name}")
    print(f"  Seed: {seed}")
    ...
    oracle = GenericOracle(dataset_name, data_dir=args.data_dir)
    dataset = GenericDataset(args, oracle, init_size=args.num_queries_per_round)

    # Initialize wandb
    if args.use_wandb:
        run = wandb.init(project='delta-cs', group=dataset_name, config=vars(args), reinit=True)
    ...
    metrics_path = os.path.join(output_path, dataset_name, f'metrics_seed{seed}.json')
    with open(metrics_path, 'w') as f:
        json.dump(result, f, indent=2, default=str)
    print(f"Results saved to: {metrics_path}")

    if rst is not None:
        rst_path = os.path.join(output_path, dataset_name, f'trajectory_seed{seed}.csv')
...
def save_aggregated_results(results: List[Dict[str, Any]], output_path: str, dataset_name: str):
    ...
    # Save
    agg_path = os.path.join(output_path, dataset_name, 'aggregated_results.json')
    ...
    # Print summary
    print(f"\n{'='*70}")
    print(f"AGGREGATED METRICS SUMMARY ({dataset_name})")
    print(f"{'='*70}")
    print(f"{'Metric':<40} {'Mean':>10} {'Std':>10}")
    print("-"*70)
    for name, stats_val in aggregated.items():
        if isinstance(stats_val, dict) and 'mean' in stats_val:
...
def main():
    ...
    # Resolve default paths based on dataset name
    ...
    if args.tb_log_dir is None:
        args.tb_log_dir = output_path

    if args.name is None:
        args.name = f'{dataset_name}_delta_cs'

    if args.task is None:
        args.task = dataset_name.lower()

    # Auto-detect sequence length from data if not specified
    if args.max_len is None or args.gen_max_len is None:
        data_path = os.path.join(args.data_dir, dataset_name, 'data.csv')
        df = pd.read_csv(data_path)
        ...
        detected_len = len(df[seq_col].iloc[0])
        print(f"Auto-detected sequence length: {detected_len}")

        if args.max_len is None:
            args.max_len = detected_len
        if args.gen_max_len is None:
            args.gen_max_len = args.max_len
    ...
    print(f"\nRunning delta-CS on {dataset_name} with {len(seeds)} seed(s): {seeds[:5]}{'...' if len(seeds) > 5 else ''}")
    print(f"Radius option: {args.radius_option}")
    ...
    if len(results) > 1:
        save_aggregated_results(results, output_path, dataset_name)
