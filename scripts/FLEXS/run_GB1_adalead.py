#!/usr/bin/env python
"""
run_GB1_adalead.py - Execute AdaLead optimization on GB1 dataset with comprehensive metrics
...
Usage:
    ...
    python run_GB1_adalead.py

    # Single run with specific seed
    python run_GB1_adalead.py --seed 42

    # Multiple runs with different seeds for randomness evaluation
    python run_GB1_adalead.py --seeds 42 123 456 789 1000

    # Use predefined seeds from file
    python run_GB1_adalead.py --seed_file rndseed.txt --num_seeds 5

    # Skip metrics computation (faster)
    python run_GB1_adalead.py --seed 42 --skip_metrics
"""
...
class GB1Landscape(flexs.Landscape):
    """
    GB1 protein fitness landscape.

    Uses empirical fitness data from combinatorial protein library.
    """
    ...
    def __init__(
        ...
        if normalize:
            self.fitness = self.fitness_raw / np.max(self.fitness_raw)
        else:
        ...
        # Wild-type sequence (full sequence)
        self.wildtype = self.sequences[0]  # First row is wild-type (VDGV -> full seq)
...
def compute_metrics(
    ...
    result.normalized_fitness_median_top128 = normalized_fitness_topk(
        queried_fitness, k=128, global_min=global_min, global_max=global_max
    )
    result.normalized_fitness_median_top256 = normalized_fitness_topk(
        queried_fitness, k=256, global_min=global_min, global_max=global_max
    )
    ...
    if model is not None:
        try:
            ...
            # Epistatic correlation and recall (need wildtype)
            result.epistatic_correlation = epistatic_score_correlation(
                queried_seqs, y_true, y_pred, wildtype
            )
            ...
            # Uncertainty metrics (if ensemble model)
            if hasattr(model, 'get_fitness_with_uncertainty'):
                y_pred_all, y_std_all = model.get_fitness_with_uncertainty(queried_seqs)
                result.miscalibration_area = miscalibration_area(y_true, y_pred_all, y_std_all)
                result.expected_calibration_error = expected_calibration_error(y_true, y_pred_all, y_std_all)
    ...
    result.global_max_hit_count = global_max_hit_count(result.max_fitness, global_max)
...
def run_single_experiment(
    seed: int,
    output_path: str = "results/GB1_AdaLead_experiments/",
    verbose: int = 2,
    run_id: Optional[int] = None,
    compute_metrics_flag: bool = True,
    data_dir: str = "/home/xux/Desktop/AlphaVariant/Benchmark/data",
    rounds: int = 4,  # 4 optimization rounds (matching ALDE)
    sequences_batch_size: int = 96,
    model_queries_per_batch: int = 2000,
    n_init_samples: int = 96  # Random initial samples (matching ALDE)
) -> Dict[str, Any]:
    """
    Run a single AdaLead optimization experiment on GB1 dataset.
    ...
    print(f"\n{'='*60}")
    print(f"Starting AdaLead optimization on GB1")
    print(f"  Seed: {seed}")
    ...
    # Load landscape
    landscape = GB1Landscape(data_dir=data_dir, normalize=True)
    ...
    print(f"Wild-type: {landscape.wildtype[:20]}...")
    print(f"Alphabet: {alphabet} (length {len(alphabet)})")
    print(f"Sequence length: {seq_length}")

    # Create output directory
    subdir = os.path.join(output_path, "GB1", "")
    ...
    if compute_metrics_flag:
        ...
        print(f"  global_max_hit_count: {metrics_result.global_max_hit_count}")
...
def save_aggregated_results(
    ...
    # Global max hit count (now stored as 0 or 1 in metrics)
    hit_count = sum(m['global_max_hit_count'] for m in metrics_list)
    ...
    # Save to CSV
    summary_path = os.path.join(output_path, 'GB1', 'aggregated_metrics.csv')
    summary_df.to_csv(summary_path, index=False)
    print(f"\nAggregated metrics saved to: {summary_path}")

    # Save complete aggregated results to JSON
    aggregated_json_path = os.path.join(output_path, 'GB1', 'aggregated_results.json')
...
def main():
    parser = argparse.ArgumentParser(
        description="Run AdaLead optimization on GB1 with CNN Ensemble",
...
Examples:
  ...
  python run_GB1_adalead.py --seed 42 --skip_metrics
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of optimization rounds after init (default: 4)"
    )
    parser.add_argument(
        "--init_samples",
        type=int,
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="results/GB1_AdaLead_experiments/",
    ...
    print(f"\nRunning AdaLead with {len(seeds)} seed(s): {seeds}")
