...
# in the input folder, generating .seq files. Then, for each .fa file, 
...
#
# Usage:
#   ./run_pipeline.sh <scripts_dir> <input_folder> <beta> <gamma> [output_folder]
#
# Example:
#   ./run_pipeline.sh ../scripts ./input_pdbs 0.8 0.5 ../custom_output
# ------------------------------------------------------------------------------

# 1. Parameter check
if [ $# -lt 4 ]; then
    echo "Usage: $0 <scripts_dir> <input_folder> <beta> <gamma> [output_folder]"
    echo "Example: $0 ../scripts ./input_pdbs 0.8 0.5 ../custom_output"
    exit 1
fi
...
for seq_file in "${seq_files[@]}"; do
    ...
    # --------------------------------------------------------------------------
    # NEW STEP: Reformat FASTA headers (awk) to make the first ID >ref, others >1, >2...
    # --------------------------------------------------------------------------
