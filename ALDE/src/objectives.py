...
class Combo(Objective):
    ...
    def __init__(self, protein, encoding, data_dir: str = 'data'):
        # Try different file names and column names for compatibility
        ...
        if 'fitness' in fitness_df.columns:
            self.y = torch.tensor(fitness_df['fitness'].values).double()
        else:
            raise ValueError("Fitness column not found in data file")

        self.y = self.y/self.y.max()

        # Store sequence column for later use
        if 'Combo' in fitness_df.columns:
            self.sequences = fitness_df['Combo'].tolist()
        elif 'AACombo' in fitness_df.columns:
            self.sequences = fitness_df['AACombo'].tolist()
        else:
            self.sequences = None

        # Load or generate encoding
        encoding_path = f'{data_dir}/{protein}/{encoding}_x.pt'
        if os.path.exists(encoding_path):
            self.X = torch.load(encoding_path)
        else:
            # Generate one-hot encoding if file doesn't exist
            if encoding == 'onehot' and self.sequences is not None:
                self.X = generate_onehot(self.sequences)
                self.X = torch.reshape(self.X, (self.X.shape[0], -1))
            else:
                raise FileNotFoundError(f"Encoding file not found at {encoding_path}")
