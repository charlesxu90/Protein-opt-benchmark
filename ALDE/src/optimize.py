...
class BayesianOptimization:
    ...
    def __init__(self,
        ...
        if self.verbose >= 1: logger.info("Initialization completed.\n")
    ...
    def optimize(self,) -> tuple:
        ...
        def save_tensors():
            ...
            """
            logger.info('Saving: {}'.format(self.savedir))
            torch.save(self.indices.cpu(), self.savedir + 'indices.pt')
            if self.verbose >= 1: logger.info('Indices saved.')
            
        if self.verbose >= 1: logger.info(f"Beginning optimization, {os.path.basename(self.savedir)}...")
        start = datetime.now()
        ...
        if self.verbose >= 1: logger.info(f"{os.path.basename(self.savedir)} | Optimization runtime: {datetime.now() - start} | Max: {max.item():.4f} | Regret: {simp_reg.item():.4f}")
    ...
    def update_trajectory(self, x, acq_val, idx):
        ...
        if self.verbose >= 3: logger.info("x index: {}, y: {}".format(idx[0], y[0]))
    ...
    @staticmethod
    def run(kwargs, seed):
        logger.info(f'Launching {kwargs.savedir.split("/")[-1]}')
        if seed is not None:
